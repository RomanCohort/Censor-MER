...
This specialization indicates MoE learns expression-specific classifiers. Figure~\ref{fig:moe_routing} visualizes the routing distribution across expressions and experts.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/moe_routing.png}
\caption{MoE Expert Routing Distribution. Each expression category shows distinct expert preferences: Expert 1 dominates happiness and surprise, Expert 2 specializes in disgust, and Expert 3 handles repression with higher rPPG correlation.}
...
\textbf{Signal Quality}: CHROM-based rPPG extraction has demonstrated reliable performance in prior literature \cite{dehaan2013robust}, with reported mean absolute error of approximately 2 BPM for heart rate estimation under controlled recording conditions. Given CASME II's standardized recording setup (200 fps, frontal view, consistent illumination), we expect comparable signal quality. The extracted signals provide chromatic features that correlate with expressions, complementing appearance and motion features. Note that ME duration (40--200ms) is shorter than typical rPPG windows, so physiological interpretability is limited (see Limitations). Figure~\ref{fig:rppg_pipeline} illustrates the three-stage CHROM extraction pipeline.
...
\item \textbf{Parameter Efficiency}: The architecture contains 68.35M parameters trained on 247 samples (ratio 276,000:1). While this appears extreme, LOSO's subject-level split provides implicit regularization---each fold trains on only 225--239 samples and tests on completely different subjects. The cross-dataset generalization (67--75\%) and consistent ablation rankings across folds suggest the model learns transferable features rather than memorizing training subjects. A parameter reduction experiment (3D ResNet-18 single-pathway, $\sim$14M parameters) achieved 85.76\% accuracy, indicating the full architecture's 2\% improvement requires the additional components (rPPG, CASANet) rather than raw parameter count. Future work should explore knowledge distillation for deployment efficiency.
\end{enumerate}
