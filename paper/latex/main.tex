...
\begin{itemize}
\item \textbf{rPPG-derived chromatic features}: +11\% contribution (statistically significant), which provides expression-correlated color signals (note: physiological interpretability limited due to ME duration constraints)
...
Micro-expressions (MEs) are involuntary facial movements lasting 40--200 ms that reveal concealed emotions \cite{ekman1969nonverbal}. Micro-expression recognition (MER) remains challenging due to brief duration, low intensity, and partial facial involvement. Recent advances in deep learning have achieved high accuracy on benchmark datasets, with state-of-the-art methods reporting 90--94\% on CASME II \cite{chen2024multiscale,ma2024hybrid}.

Neuroscience research shows that human facial expression perception involves dual neural pathways: a fast subcortical route (superior colliculus $\rightarrow$ pulvinar $\rightarrow$ amygdala) for rapid coarse processing \cite{morris1999subcortical}, and a slow cortical route (V1 $\rightarrow$ V2 $\rightarrow$ V4 $\rightarrow$ fusiform face area) for fine-grained analysis \cite{kanwisher1997fusiform}. This dual-pathway architecture suggests a possible improvement for MER systems.
...
\textbf{Handcrafted Era}: LBP-TOP achieved 63.3\% on CASME II using spatiotemporal texture encoding \cite{zhao2014lpbtop}. MDMO quantified motion patterns, reaching 66.4\% \cite{liu2016mdmo}.
...
Three expert networks with noisy top-2 gating. The number of experts ($E=3$) is determined through empirical ablation (Table~\ref{tab:expert_count}), balancing accuracy and routing efficiency. $E=3$ achieves the highest accuracy on simplified models while maintaining computational efficiency:
...
\toprule
\textbf{Method} & \textbf{Year} & \textbf{Accuracy} & \textbf{Std} & \textbf{Protocol} & \textbf{Verified} \\
\midrule
LBP-TOP + SVM & 2014 & 63.3\% & -- & LOSO & \checkmark \\
HOG-TOP + SVM & 2015 & 65.2\% & -- & LOSO & \checkmark \\
MDMO & 2016 & 66.4\% & -- & LOSO & \checkmark \\
STRN & 2019 & 78.6\% & -- & LOSO & \checkmark \\
Dual-Stream CNN & 2020 & 82.1\% & -- & LOSO & \checkmark \\
OFF-ApexNet$^\dagger$ & 2022 & 87.64\% & -- & Not specified & $\times$ \\
MER-Transformer & 2022 & 87.3\% & -- & LOSO & \checkmark \\
MEViT & 2023 & 88.2\% & -- & LOSO & \checkmark \\
Multi-scale Temporal & 2023 & 89.1\% & -- & LOSO & \checkmark \\
\textbf{Censor (Ours)} & 2024 & \textbf{87.74\%} & $\pm$12.76\% & \textbf{Standard LOSO (24 folds)} & \checkmark \\
\bottomrule
\end{tabular}

$^\dagger$Protocol not specified in original paper; direct comparison may be misleading due to potential data leakage.
\end{table}
...
\midrule
LBP-TOP & 63.3\% & LOSO (verified) & No \\
OFF-ApexNet & 87.64\% & Not specified & Partial \\
Multi-scale 3D ResNet & 91.35\% \cite{chen2024multiscale} & Not specified & No \\
Hybrid Attention-3DNet & 93.79\% \cite{ma2024hybrid} & Not specified & No \\
...
\textbf{Statistical Significance}: Since LOSO folds evaluate the same model architecture across different test subjects, we use \textbf{Wilcoxon signed-rank test} (paired, non-parametric) to compare ablation variants, with Holm-Bonferroni correction for correlated comparisons. Paired analysis accounts for the dependency that each fold tests the same training configuration. The rPPG and CASANet contributions are statistically significant: No-CASANet ($W=19$, $p_{\text{adj}}<0.004$), No-rPPG ($W=17$, $p_{\text{adj}}<0.004$). The MoE contribution (+2.5\%) shows a trend but does not reach statistical significance after correction: Fast-only ($W=87$, $p_{\text{adj}}=0.156$). We also report \textbf{bootstrap 95\% confidence intervals} (10,000 resamples, bias-corrected accelerated (BCa) method, preserving paired structure): rPPG $[+6.2\%, +15.1\%]$, CASANet $[+5.8\%, +13.9\%]$, MoE $[-0.3\%, +5.2\%]$. Cohen's $d$ shows medium effects for rPPG ($d=0.58$) and CASANet ($d=0.53$), and a negligible effect for MoE ($d=0.16$). The difference between Dual-no-MoE and Fast-only is not significant ($W=102$, $p=0.412$), confirming dual-pathway fusion alone provides no benefit.

\textbf{Statistical Power Analysis}: With $n=24$ LOSO folds and observed standard deviation $\sigma \approx 12.8\%$, the minimum detectable effect size at 80\% power (two-tailed $\alpha=0.05$) is approximately $d=0.58$ (medium), corresponding to $\Delta \approx 7.5\%$ accuracy difference \cite{cohen1988statistical}. This \textbf{post hoc sensitivity analysis} (not an a priori power calculation) indicates effects smaller than 7--8\% (such as MoE's +2.5\%) cannot be reliably detected. The rPPG (+10.76\%) and CASANet (+9.77\%) effects exceed this threshold and are thus detectable, while MoE remains inconclusive due to dataset size limitations rather than definitive ineffectiveness.
